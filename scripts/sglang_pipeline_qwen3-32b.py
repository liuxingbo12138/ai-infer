#!/usr/bin/env python3
"""
sglang 统一压测 Pipeline (多轮, 支持 session)
===============================================
流程: 启动服务 → 健康检查 → 多轮逐级压测 → 日志收集 → 结果汇总 → 清理

用法:
    # 首次运行: 启动服务 + 压测 (完成后自动 kill 服务)
    python3 sglang_pipeline_qwen3-32b.py

    # 只启动服务, 不跑压测 (服务保持运行)
    python3 sglang_pipeline_qwen3-32b.py --server-only

    # 只跑压测, 不启动服务 (复用已运行的服务, 结果写入同一 session 目录)
    python3 sglang_pipeline_qwen3-32b.py --no-server

    # 只看结果 (从当前 session 的 summary.json 读取并打印表格)
    python3 sglang_pipeline_qwen3-32b.py --show-summary

    # 查看指定 session 的结果
    python3 sglang_pipeline_qwen3-32b.py --show-summary bench_20260302_092317
"""
...
SESSION_FILE = "/data/logs/sglang/.current_session"
LOG_BASE_DIR = "/data/logs/sglang"
...
def start_server(output_dir):
    """后台启动 sglang server，日志重定向到 server.log。"""
    log_path = os.path.join(output_dir, "server.log")
    log_file = open(log_path, "w")
    print(f"[Pipeline] 启动 sglang server ...")
...
def kill_server(proc):
    """终止 sglang server 进程组。"""
    if proc and proc.poll() is None:
        print(f"\n[Pipeline] 正在终止 sglang server (pid={proc.pid}) ...")
        try:
...
def run_benchmark(concurrency, input_len, output_len, round_dir):
    """运行单轮压测，返回 jsonl 文件路径。"""
    num_prompts = concurrency * 5
    jsonl_path = os.path.join(round_dir, f"bench_c{concurrency}.jsonl")
    ...
    print(f"[Pipeline] ✓ 并发={concurrency} 完成, 结果: {jsonl_path}")
    return jsonl_path


# ==================== 结果处理 ====================

def extract_metrics(jsonl_path):
    """从 jsonl 结果文件中提取关键指标。"""
    try:
        ...
        print(f"[Pipeline] ⚠ 解析 {jsonl_path} 失败: {e}")
        return None
...
def parse_args():
    parser = argparse.ArgumentParser(description="sglang 统一压测 Pipeline")
    ...
    try:
        ...
        for i, round_cfg in enumerate(TEST_ROUNDS, 1):
            ...
            for c in concurrency_list:
                jsonl_path = run_benchmark(c, input_len, output_len, round_dir)
                if jsonl_path:
                    metrics = extract_metrics(jsonl_path)
                    if metrics:
